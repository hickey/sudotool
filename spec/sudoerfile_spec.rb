...
EOF
  ...
  describe '#write' do
    ...
    it 'creates a correctly formatted expires header when set to never' do
      @sudo = SudoTool::SudoerFile.new @test_file.path
      @sudo.expiration = :never
      @sudo.write
      
      @sudotest = SudoTool::SudoerFile.new @test_file.path
      @sudotest.expiration.should eql :never
    end
